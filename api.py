...
def detect_faces_dnn_api(image, net, conf_threshold=0.7):
    ...
    Retorna una lista de bounding boxes (x, y, w, h).
    """
    (h, w) = image.shape[:2]
...
def predict_emotion(face_img):
    """
    Dada una imagen de rostro (BGR), predice la emoción.
    """
...
# Diccionario global para rostros registrados: nombre -> encoding
registered_faces = {}
...
async def register_face(name: str = Form(...), file: UploadFile = File(...)):
    ...
    # Convertir RGBA a RGB si procede
    if image.shape[2] == 4:
        image = cv2.cvtColor(image, cv2.COLOR_RGBA2RGB)
    
    # Detección usando face_recognition (HOG)
...
async def identify_face(
    ...
    mode: str = Form("Deteccion completa")  # Valor por defecto
):
    """
    Recibe una imagen y un modo, y realiza acciones diferentes:
      - deteccion completa: detecta 1er rostro, reconoce, predice emoción
      - deteccion emociones: solo predice emoción
      - deteccion rostros: detecta 1er rostro y reconoce sin emoción
      - no hacer nada: no procesa
    """
    contents = await file.read()
    try:
        image = np.array(Image.open(io.BytesIO(contents)))
    ...
    if mode_lower == "no hacer nada":
        ...
        # Asumimos que toda la imagen es un rostro o al menos tratamos de predecir la emoción
        # (Si quieres recortar un rostro detectado, llama a detect_faces_dnn_api e identifica el 1º)
        ...
        # Detectar un rostro con DNN + reconocimiento (sin emoción)
        boxes = detect_faces_dnn_api(image, net, conf_threshold=0.7)
        if len(boxes) == 0:
            return JSONResponse(content={"error": "No se detectó rostro en la imagen."}, status_code=400)
